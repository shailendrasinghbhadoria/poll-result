...
const Vote: React.FC<VoteProps> = ({ options, onVote, viewWinner }) => {
    ...
    return (
        ...
        {viewWinner===false ? <div className="vote-container w-40 h-50" style={{display: "flex",justifyContent: "space-evenly"}}>
            
        { options.map((item)=>(<div key={item.id} data-testid={`option-1`} className="card mr-10 ml-10 w-100">
        
            <section className="layout-row align-items-center justify-content-center mr-10 ml-10 pr-10 pl-10">
            
                <h3 data-testid={`choice-1}`}>{item.text}</h3>
            
            </section>
        
            <section className="layout-row align-items-center justify-content-center mr-10 ml-10 mb-10 pr-10 pl-10">
            
                <button onClick={()=>onVote(item.id)} disabled={false} data-testid={`vote-button-1`}>
                    Vote
                </button>
            </section>
        ...
        {/* <div key={2} data-testid={`option-2`} className="card mr-10 ml-10 w-100">
            ...
            <section className="layout-row align-items-center justify-content-center mr-10 ml-10 pr-10 pl-10">
                ...
                <h3 data-testid={`choice-2}`}>{options[1].text}</h3>
